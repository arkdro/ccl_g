...
func Test_compare_neighbours_2(t *testing.T) {
	merged := result.Build_merge_ccl_result_2()
	width := 7
	height := 4
	connectivity := 6
	...
	for exp_key, exp_node := range expected_g {
		result := compare_neighbours(graph, exp_key, &exp_node)
		if !result {
			t.Errorf("compare neighbors, conn 6, error. Key: %+v, node: %+v\n",
				exp_key, exp_node)
		}
	}
}
...
func Test_compare_cells_1(t *testing.T) {
	...
	expected_g := result.Build_expected_g_result_c8()
	for exp_key, exp_node := range expected_g {
		result := compare_cells(graph, exp_key, &exp_node)
		if !result {
			t.Errorf("compare cells, conn 8, error. Key: %+v, node: %+v\n",
				exp_key, exp_node)
		}
	}
}

func Test_compare_cells_2(t *testing.T) {
	merged := result.Build_merge_ccl_result_2()
	width := 7
	height := 4
	connectivity := 6
	...
	for exp_key, exp_node := range expected_g {
		result := compare_cells(graph, exp_key, &exp_node)
		if !result {
			t.Errorf("compare cells, conn 6, error. Key: %+v, node: %+v\n",
				exp_key, exp_node)
		}
	}
}
...
func Test_compare_labels_2(t *testing.T) {
	merged := result.Build_merge_ccl_result_2()
	width := 7
	height := 4
	connectivity := 6
	...
	result := compare_labels(graph, expected_g)
	if !result {
		t.Error("compare labels, conn 6, error")
	}
}
